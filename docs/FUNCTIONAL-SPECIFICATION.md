...
Ne font pas partie de cette version, et ne devront pas être introduits sans remise en cause
explicite des principes ci-dessus :

- Compte utilisateur, synchronisation multi-appareils, sauvegarde distante.
- Publicité, achat intégré, classement, badge de performance, notion d'échec.
- Analytique, télémétrie, suivi d'usage.
- Gestion de tâches ou de projets (l'application minute, elle n'organise pas).
- Partage social des statistiques.
- Widget Android (non réalisé à ce jour), montre connectée.
- Export des données par l'utilisateur — évolution envisageable, non spécifiée ici.
