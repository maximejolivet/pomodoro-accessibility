import type { Dictionary } from '../i18n.model';

export const it: Dictionary = {
  'state.running': 'In corso',
  'state.paused': 'In pausa',
  'state.finished': 'Finito',
  'state.ready': 'Pronto',
  'state.extra': 'Tempo extra',
  'mode.custom': 'Personalizzato',
  'mode.extra': '+5 min',
  'cycle': 'Ciclo {n}/{total}',

  'action.start': 'Avvia',
  'action.pause': 'Pausa',
  'action.resume': 'Riprendi',
  'action.reset': 'Azzera',
  'action.settings': 'Impostazioni',
  'action.close': 'Chiudi',
  'action.minus': 'Togliere un minuto',
  'action.plus': 'Aggiungere un minuto',
  'action.lock': 'Blocca il quadrante',
  'action.unlock': 'Sblocca il quadrante',
  'action.dialName': 'Durata',
  'action.dialHint': 'trascina sul quadrante, usa le frecce o i pulsanti − e + per regolare i minuti; premi per avviare o mettere in pausa',
  'action.accessibilityInfo': 'Visualizza documentazione accessibilità RGAA 4.1',
  'action.accessibilityShort': 'Accessibilità RGAA',

  'unit.minutes': 'minuti',

  'color.purple': 'Viola',
  'color.red': 'Rosso',
  'color.orange': 'Arancione',
  'color.amber': 'Ambra',
  'color.lime': 'Verde lime',
  'color.green': 'Verde',
  'color.cyan': 'Ciano',
  'color.blue': 'Blu',
  'color.magenta': 'Magenta',
  'color.gray': 'Grigio',

  'tab.modes': 'Modalità',
  'tab.stats': 'Statistiche',
  'tab.settings': 'Impostazioni',

  'settings.dark': 'Modalità scura',
  'settings.sound': 'Suono',
  'settings.haptics': 'Vibrazione',
  'settings.hapticsHint': 'Uno schema diverso a ogni traguardo',
  'settings.autoExtra': '+5 min automatici alla fine',
  'settings.autoChain': 'Concatena lavoro → pausa',
  'settings.autoChainHint': 'Pausa lunga ogni {n} cicli',
  'settings.keepAwake': 'Schermo sempre acceso',
  'settings.visualAlert': 'Avviso visivo',
  'settings.visualAlertHint': 'Un lampo colorato ai traguardi e alla fine',
  'settings.visualOff': 'Nessuno',
  'settings.visualSoft': 'Leggero',
  'settings.visualStrong': 'Forte',
  'settings.speech': 'Annuncio vocale',
  'settings.speechHint': 'Il tempo restante detto a voce alta',
  'settings.speechOff': 'Nessuno',
  'settings.speechMilestones': 'Traguardi',
  'settings.speechMinutes': 'Ogni minuto',
  'settings.language': 'Lingua',
  'settings.languageAuto': 'Auto',
  'settings.listen': 'Ascolta i suoni',
  'settings.listenOne': 'Riproduci il suono {label}',
  'settings.duration': 'Durata',
  'settings.goal': 'Obiettivo giornaliero',

  'preset.pomodoro': 'Pomodoro',
  'preset.break': 'Pausa',
  'preset.longBreak': 'Pausa lunga',
  'preset.focus': 'Focus ADHD',
  'preset.new': 'Nuova modalità',
  'preset.edit': 'Modifica {name}',
  'preset.name': 'Nome',
  'preset.kind': 'Tipo',
  'preset.kind.focus': 'Lavoro',
  'preset.kind.break': 'Pausa',
  'preset.kind.longBreak': 'Pausa lunga',
  'preset.color': 'Colore',
  'preset.save': 'Salva',
  'preset.cancel': 'Annulla',
  'preset.delete': 'Elimina',
  'preset.restore': 'Ripristina le modalità predefinite',
  'preset.confirm': 'Tocca di nuovo per confermare',

  'routine.section': 'Routine',
  'routine.sectionHint': 'Una sequenza di passi che si concatenano da soli',
  'routine.new': 'Nuova routine',
  'routine.edit': 'Modifica la routine {name}',
  'routine.summary': '{n} passi · {m} min',
  'routine.start': 'Avvia {name}',
  'routine.step': 'Passo {n} di {total}: {name}, {m} minuti',
  'routine.stepDone': 'completato',
  'routine.stepCurrent': 'in corso',
  'routine.finished': 'Routine completata: {name}',
  'routine.quit': 'Esci dalla routine',
  'routine.delete': 'Elimina la routine',
  'routine.icon': 'Pittogramma',
  'routine.steps': 'Passi',
  'routine.stepAdd': 'Aggiungi un passo',
  'routine.stepEdit': 'Modifica {name}',
  'routine.stepUp': 'Sposta su {name}',
  'routine.stepDown': 'Sposta giù {name}',
  'routine.stepRemove': 'Elimina {name}',
  'routine.stepUntitled': 'Passo senza nome',
  'routine.counted': 'Conta nel focus di oggi',
  'routine.countedHint': 'Spento per lavarsi i denti, acceso per studiare',
  'routine.restore': 'Ripristina le routine predefinite',
  'routine.empty': 'Nessuna routine. Crea una sequenza di passi che si concatenano.',
  'routine.morning': 'Routine del mattino',
  'routine.morning.dress': 'Vestirsi',
  'routine.morning.breakfast': 'Colazione',
  'routine.morning.teeth': 'Denti',
  'routine.morning.bag': 'Zaino',
  'routine.homework': 'Compiti',
  'routine.homework.setup': 'Prepararsi',
  'routine.homework.work': 'Lavorare',
  'routine.homework.pause': 'Pausa',
  'routine.homework.tidy': 'Riordinare',

  'stats.focus': 'Focus oggi',
  'stats.sessions': 'Sessioni',
  'stats.streak': 'Serie',
  'stats.streakValue': '{n} g',
  'stats.week': 'Ultimi 7 giorni',
  'stats.weekDay': 'Giorno',
  'stats.weekMinutes': 'Minuti di focus',
  'stats.weekLabel': '{day}: {m} min di focus',
  'stats.recent': 'Ultime sessioni',
  'stats.empty': 'Ancora nessuna sessione. Avvia un timer!',
  'stats.completed': 'Completata',
  'stats.interrupted': 'Interrotta',
  'stats.clear': 'Cancella la cronologia',
  'stats.goal': 'Obiettivo di oggi',
  'stats.goalReached': 'Obiettivo raggiunto!',

  'speech.oneMinute': 'Ancora un minuto',

  'notif.milestoneTitle': 'Ancora {m} minuti',
  'notif.milestoneBody': 'Il tempo scorre, tieni duro.',
  'notif.endTitle': 'Tempo scaduto',
  'notif.endBodyExtra': '+{m} min per finire.',
  'notif.endBody': 'Fai una pausa.',
  'notif.extraEndTitle': 'Tempo extra finito',
  'notif.nextBody': 'Prossimo: {name}',
  'notif.channel.milestone': 'Traguardo {m} min',
  'notif.channel.end': 'Fine del timer'
};
